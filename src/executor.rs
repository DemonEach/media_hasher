...
impl<'a> Executor<'a> {
    ...
    fn rename_specific_files(&self) {
        ...
        for file in self.options.get_files() {
            ...
            debug!("Got full file path: {:?}", file_path);
    ...
    fn rename_all(&self) {
        ...
        let paths = self.get_img_files(curr_dir);
        if paths.len() != 0 {
    ...
    fn get_img_files(&self, curr_dir_path: PathBuf) -> Vec<PathBuf> {
        let paths = fs::read_dir(curr_dir_path).unwrap();

        let mut result_files: Vec<PathBuf> = vec![];
        for entry in paths {
            if let Ok(entry) = entry {
                if self.is_media_file_type(&entry.path()) {
    ...
    fn is_media_file_type(&self, path: &PathBuf) -> bool {
        if path.is_file() {
